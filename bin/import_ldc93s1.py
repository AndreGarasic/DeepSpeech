...
#            ".", ""
#        )
local_file='test_audio.wav'
data_dir='test1'
    df = pandas.DataFrame(
        data=[(os.path.abspath(local_file), os.path.getsize(local_file), "Upali ledice")],
        columns=["wav_filename", "wav_filesize", "transcript"],
    )
    df.to_csv(os.path.join(data_dir, "proba_test.csv"), index=False)
